...
Ragdoll::Ragdoll(Vector2f _pos, b2World& _world) {
	...
	// Cabeza
	bdyDef_Rag[0].position = b2Vec2(_pos.x, _pos.y + 0.1f);
	// Torso
	bdyDef_Rag[1].position = b2Vec2(_pos.x, _pos.y + 0.65f);
	// Brazo Izq
	bdyDef_Rag[2].position = b2Vec2(_pos.x - 0.32f, _pos.y + 0.5f);
	// Brazo Der
	bdyDef_Rag[3].position = b2Vec2(_pos.x + 0.32f, _pos.y + 0.5f);
	// Pierna Izq
	bdyDef_Rag[4].position = b2Vec2(_pos.x - 0.09f, _pos.y + 1.5f);
	// Pierna Der
	bdyDef_Rag[5].position = b2Vec2(_pos.x + 0.09f, _pos.y + 1.5f);

	//Definimos al radgoll
	for (int i = 0; i < 6; i++) {
		bdyDef_Rag[i].type = b2_dynamicBody;
		bdy_Rag[i] = _world.CreateBody(&bdyDef_Rag[i]);
	...
	for (int i = 0; i < 5; i++) {
		...
		jointRag[i] = (b2DistanceJoint*)_world.CreateJoint(&jointRagDef[i]);
	}
...
void Ragdoll::Draw(RenderWindow& wnd) {
	for (int i = 0; i < 6; i++) {
		ragdollParts[i]->Draw(&wnd);
	}
}

void Ragdoll::ApplyForce(Vector2f _mousePos) {
	for (int i = 0; i < 6; i++) {
		bdy_Rag[i]->ApplyForceToCenter(b2Vec2(_mousePos.x * 5, _mousePos.y * 5), false);
	}
}
...
float Ragdoll::RadToDeg(float _radians) {
